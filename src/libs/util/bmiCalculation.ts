...
export const calculateWeightDifference = (height: number, weight: number) => {
  ...
  const muscularDifference = (muscularWeight - weight)
    .toFixed(decimalPlaces)
    .toString()
    .padStart(decimalPlaces + integerPlaces, '0');
  const slimDifference = (slimWeight - weight)
    .toFixed(decimalPlaces)
    .toString()
    .padStart(decimalPlaces + integerPlaces, '0');
  const normalDifference = (normalWeight - weight)
    .toFixed(decimalPlaces)
    .toString()
    .padStart(decimalPlaces + integerPlaces, '0');
...
export const targetCalculateWeightDifference = (height: number, weight: number, targetBody: string) => {
  ...
  const targetDifference = (targetWeight - weight)
    .toFixed(decimalPlaces)
    .toString()
    .padStart(decimalPlaces + integerPlaces, '0');
